...
| `stage4.md` | Stage 4 plan: memory (done) + inter-agent — conversation-state, event-bus, cross-agent chains all built; **memory-driven multi-agent orchestration ✅ DONE (closes #290): `coordinator-agent` + the read-only `brief` cross-agent query + bounded confidence loop** | Any Stage-4 (dialog state / inter-agent / event-bus / coordinator / multi-domain) work |
...
| `migration-25-boot4.md` | Platform version migration: Java 21→25 LTS, Spring Boot 3.4→4.0.7, Spring AI 1→2, Jackson 2→3 — **✅ DONE 2026-07-05** (full change inventory + rationale); remaining follow-up = the build/CI performance pass (§Build/CI performance) | The JVM/Boot/Spring-AI upgrade detail, or planning the build/CI perf pass |
