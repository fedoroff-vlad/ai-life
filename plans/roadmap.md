...
- **Stage 5 — real LLM (IN PROGRESS, #199):** llm-gateway providers (mock / anthropic / openai-compatible) + Langfuse traces — ✅ built. Real-model validation now **unblocked via local Ollama** (`qwen2.5:7b` + `nomic-embed-text`, free, owner's box) — zero-code `LLM_PROVIDER=openai-compatible`. First **golden tests** landed: `finance-agent` `GoldenRoutingTest` (opt-in `@Tag("golden")` + `GOLDEN_LLM`, skipped in CI) asserts **structure, not text** against the live model. Proved the finance routing spine on `qwen2.5:7b` and fixed two real format-drift findings (flattened tool shape tolerated in `IntentRouter`; `action` enum pinned in the classifier prompt). Remaining: golden coverage for the other agents + orchestrator agent-routing; pick a stronger default model if 7B quality is short. Anthropic/DeepSeek keys remain the paid alternative.
