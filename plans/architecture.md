...
- **Soft-fail, but never silently (transparency doctrine, road-test [#485](https://github.com/fedoroff-vlad/ai-life/issues/485)).** Every best-effort step (a memory recall, a render/store, a note write, a tool gather) is *robust* — it soft-fails and the flow continues rather than sinking the whole reply. But robust ≠ mute: when a best-effort step **on the reply path** fails, the reply must **say so discreetly** (a trailing `⚠️ …` note via `agent-runtime` `DegradedNotice`) instead of pretending success or returning silence — the owner must be able to tell a real "nothing found" from "the store was down". The mechanism owns the consistent rendering; the wording is small per-flow copy in the user's language. Off-the-reply-path best-effort steps (fire-and-forget `observe`, an idempotent cron ensure) stay silent — there is no reply to annotate. First consumer: `briefing-agent`'s board-store fallback.
  - Scenario: **degraded step is surfaced.** WHEN a capture/store/render step soft-fails on the reply path → THEN the reply carries an explicit `⚠️` degraded-state note, not silence or a false "готово". (e.g. briefing board store fails → text-only reply **with** the notice, and **no** fake board link.) (asserted by `DegradedNoticeTest`)
  - Scenario: **nothing degraded, nothing said.** WHEN every step succeeds → THEN no degraded note is added (the notice is append-only on real failure). (asserted by `DegradedNoticeTest`)
- **Sanity spot-checks — cheap guards where a wrong number is expensive (road-test [#485](https://github.com/fedoroff-vlad/ai-life/issues/485)).** Beyond honest degradation, the assistant runs *cheap, deterministic* guards on the data it is about to present and **flags** an obviously-wrong result rather than showing it clean. The guard is a pure invariant check (no LLM), the flag reuses the same discreet `⚠️` surface (`DegradedNotice.append`), and it flags rather than blocks unless the value is truly impossible. Only genuine invariants qualify — e.g. an event that ends before it starts, a receipt dated in the future; *not* values that merely look large (a big rent is fine) or a category net-negative from legitimate refunds. First consumers: finance receipt future-date caution, calendar `create_event` end-after-start.
  - Scenario: **impossible value flagged.** WHEN a produced result fails a deterministic sanity invariant (future-dated receipt, end-before-start event) → THEN it is flagged in the reply (a `⚠️` caution, or a hard `ok=false` when creating an impossible row), not presented as clean (asserted by `ReceiptFlowTest` / `ActionControllerTest`).
  - Scenario: **plausible value untouched.** WHEN the result satisfies the invariants → THEN no caution is added (guards are append-only on a real violation, never on a merely-large-but-valid number) (asserted by `ReceiptFlowTest`).
...
│   ├── docker-compose.dev.yml      # infra only (PG/Radicale/SeaweedFS/Langfuse)
│   ├── .env.example
...
Per-channel model picks + rationale + rollout live in **[model-strategy.md](model-strategy.md)** (the MoE-first stack for the Mac); that file + `.env.mac.example` stay the tag SSOT, not this paragraph.
...
- **AGENT.md** at each `domains/<domain>/<name>-agent/` — **our own manifest format, not an Anthropic convention** (Anthropic specs `SKILL.md` only; there is no Anthropic spec for a per-module agent manifest, and the closest open standard is the community root-level [AGENTS.md](https://agents.md), which this deliberately is not): frontmatter `name, description, version, port, mcp, skills, triggers, intents` + EN system-prompt body. Spring Boot reads it at startup and registers the agent's skills + MCP bindings; `intents` / `triggers` feed the orchestrator's classifier (data-driven routing).
...
- **Durable inbound (#633):** `libs/inbox` (`bus.inbox`) is the outbox's inbound mirror — the Telegram ingress persists-before-process and a poll-based redriver replays anything left un-`PROCESSED` after a downstream outage/restart, so a synchronous-path failure no longer drops the user's message (dedup on `update_id`, bounded → `DEAD`). Same at-least-once primitive, opposite direction. See [platform.md](platform.md) §Durable inbound inbox + `libs/inbox/README.md`.
- LLM: only via `libs/llm-client`. MCP: only via `libs/mcp-client`.
- **Resilience on the synchronous LLM path (#631):** `libs/llm-client` wraps every unary gateway call in a response timeout + a transient-only retry (connect/timeout/5xx, exponential backoff) + a circuit breaker (`CB(Retry(call))`) that fails fast once the gateway is sustainedly unhealthy — a 4xx is never retried nor counted, and streaming is breaker-guarded but not retried. Knobs under `ailife.llm-client.resilience.*` (see `libs/llm-client/README.md`). The async outbound path already has its at-least-once backstop in `libs/event-bus` (outbox); durable **inbound** buffering is the separate [#633](https://github.com/fedoroff-vlad/ai-life/issues/633).
...
- Telegram: one bot; identify by telegram_user_id → user_id; group chat supported for household commands. Onboarding is **invite-only** (deep-link `start` token — a bot can't DM first), owner-gated.
- **Identity & membership (ADR-0001, Accepted 2026-07-31):** the `user_id → household_id` link is **1:N**, not 1:1 — a user owns a **personal household** and can be an approved **member** of shared (family) households via `core.household_members`. An item lives in exactly one household = its visibility boundary (**tenant routing**): private → personal household, shared → family household; a member reads the **union** over their memberships. A friend registers into their own personal household → full isolation. Owner is the admin (no separate admin surface). A contact (`core.people`) may optionally link to the operator it became via nullable `people.user_id` (ADR-0001 item 6, 2026-08-04) — an identity link, orthogonal to item *visibility* (that is household routing / ADR-0002). See [adr/ADR-0001](adr/ADR-0001-identity-membership-scope.md). *(Memory keeps its existing owner-tag `scope` form below until reconciled.)*
- **Sharing is a reusable cross-domain capability (ADR-0002, Accepted 2026-08-01):** the personal/shared routing built for calendar is generalised into a shared leaf module **`libs/sharing`** (`SharingResolver` write-engine + `DefaultSharingPolicy` extension point + `SharingContext` + `ProfileSharingClient`), taken by **all** domain agents *and* read-only web services — one mechanism, no duplication. `SharingScope` lives in `contracts/common`. Each domain plugs its own tweak in a same-named **`sharing/`** package (`<Domain>SharingPolicy` = "what is shared here"). **The mechanism (which household a row lands in / which set a read spans) is deterministic — a privacy boundary, never LLM-decided;** only the *default-when-unspecified* is judgement. That default **has graduated** (item 8, DS-0…DS-4, 2026-08-05) from a static per-domain rule to one that **learns the owner's past explicit choices** — a `LearnedSharingPolicy` decorating each domain's static policy, a deterministic majority vote over memory-service's `memory.sharing_decision` tally (trusted only when deep + decisive, else the static rule), all through the **same `DefaultSharingPolicy` seam** (ADR-0001 item 7). Still deterministic, never LLM — the learning only sharpens the *default*, not the mechanism. Confirm-on-ambiguity (**DS-N**) is **shipped** (2026-08-07, epic complete): when a domain's default is genuinely ambiguous it defers + asks "личное или общее?" once via the reusable `SharingConfirm` (pending-action + `/resume`), then records the reply as the learn signal (tasks/finance/docs ask; calendar's inter-agent write + nutrition's deterministically-shared basket opt out by design). Calendar is the reference implementation. See [adr/ADR-0002](adr/ADR-0002-sharing-shared-capability.md) + PATTERNS.md "Recipe: add sharing to a domain".
- **Object store = SeaweedFS behind the S3 API (ADR-0009, Accepted 2026-09-24):** media blobs live in a single-container S3 store; config names the **protocol, not the product** (`media.s3.*` / `S3_*`), and the integration tests run the **same store as the deploy**. MinIO held this slot from Stage 0 until its server image stopped being publicly pullable from every registry it used (Docker Hub / quay.io / ghcr, 2026-09-24) — a red CI *and* a stack a fresh host could not start. The `io.minio` SDK stays as a plain S3 client. Anything beyond put/get/remove (presigned URLs, multipart, lifecycle) must be verified against SeaweedFS before it is relied on. See [adr/ADR-0009](adr/ADR-0009-object-store.md).
- **Personalization profile is a reusable cross-domain capability (ADR-0005, Accepted 2026-08-28):** the repeated per-member `(household_id, owner_id)` preference-profile pattern (5 domains: briefing/creator/nutrition/travel/stylist) is generalised into a shared mechanism — a light leaf **`libs/profile`** (`ProfileScope` write-scope helper + `ProfileScopeResolver` with the one resolution rule `self → own household-default → family/shared household-default → empty`, reusing `libs/sharing`'s `ProfileSharingClient` identity read) + a `PersonalizationProfiler` extract→scope→write **template in `agent-runtime`** (it needs the LLM/SKILL deps). **The store stays per-domain** (heterogeneous jsonb, hot-path local reads, schema-per-domain) — only the mechanism lifts. Each domain plugs a thin same-named **`profile/`** adapter (its typed client + `SetInput` builder + any post-step, e.g. briefing's geocode); the SKILL stays in domain skills. Generalises #490 FO-3's family-default inheritance to all five domains. Coach's `(household, subject)` per-subject profile is a different model — out of scope. See [adr/ADR-0005](adr/ADR-0005-personalization-profile-capability.md) + PATTERNS.md "Recipe: add a personalization profile to a domain".
