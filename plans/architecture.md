...
- **Platform migration sequencing (owner, 2026-07-03):** finish the ambient-capture / second-memory stage FIRST, then do the Java 25 + Spring Boot 4 + Spring AI 2 migration as its own dedicated stage (not mixed with feature work). **The ambient-capture stage is now complete (AC-1..5, 2026-07-04), so the migration is the next stage.** Plan + rationale: [migration-25-boot4.md](migration-25-boot4.md).
