...
├── platform/   shared cross-cutting SERVICES: gateway-telegram, orchestrator, llm-gateway, memory-service, profile-service, scheduler-service, notifier-service
├── agents/     reasoners (own Spring Boot app each): domain agents (calendar/finance/tasks) AND cross-domain specialists (future: search/researcher, stylist)
├── mcp/        ALL MCP servers, flat. domain-MCP (owns a schema): mcp-caldav, mcp-finance, mcp-tasks, mcp-ics-import, mcp-money-pro-import. capability-MCP (schema-less shared tool, any agent binds): mcp-media-processing, and future mcp-weather, mcp-web, …
└── skills/     instructions (prompts). skills/<domain>/<name>/ per domain; skills/shared/<name>/ for cross-cutting behaviour an agent opts into via its skills-classpath. _registry.yaml autogenerated
```
Adding an agent/MCP = new folder from template (PATTERNS.md) + register in root `pom.xml` `<modules>` + docker-compose. No edits to existing services.
...
| A **domain agent** or a cross-domain **specialist** (search/stylist) | `agents/<name>` | reasoner = AGENT.md + skills + bound MCP |
| A **domain tool** that owns data | `mcp/<name>` (domain-MCP) | has a schema + Liquibase feature |
| A **shared tool** over an external API (weather, web search) | `mcp/<name>` (capability-MCP) | **no schema**; bound by any agent; PATTERNS "add a capability-MCP" |
| A **behaviour/instruction** (prompt) for one domain | `skills/<domain>/<name>/` | loaded by that domain's agent |
| A **cross-cutting behaviour** reused by several agents | `skills/shared/<name>/` | each agent opts in via its `agent.skills-classpath` glob |
...
Rule of thumb: **tools = MCP, reasoning = agent, instructions = skill, editable rules = data.** Don't fragment `mcp/` or `agents/` into sub-folders by kind — the domain/capability (or domain/specialist) distinction lives in the module README header + whether it owns a schema, not in the filesystem.
