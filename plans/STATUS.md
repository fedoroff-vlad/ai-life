...
- **inventory domain ([inventory.md](inventory.md)) — IN FLIGHT; store landed, agent side next (owner pick 2026-09-22).**
  ...
  vision-captioned item in it, close it). Detail → [inventory.md](inventory.md) + [HISTORY.md](HISTORY.md).
  **Next: IN-d** — the QR label image + the `libs/doc-render` container card (what the owner prints and
  what a scan will show). Then IN-e ("где лежит X") and IN-f (the scan path: `box_` prefix on the gateway's
  `/start` + a photographed label). Label printing stays deferred until
