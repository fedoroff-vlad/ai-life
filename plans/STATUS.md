...
## Now
- **inventory domain ([inventory.md](inventory.md)) — IN FLIGHT; the scan path is closed and the domain is correctable in chat (IN-a…IN-g1 shipped + goldens), the item-level edits are next.**
  ...
  **Next:** **IN-g2/g3** (the item half — "убери оттуда X" on the same runner, "добавь в B-07 гирлянду"
  without reopening a session) · **IN-e2** (semantic recall — an item note seed on the write path,
  mirroring docs SB-5). Label printing stays deferred until the printer exists — the owner is buying in
