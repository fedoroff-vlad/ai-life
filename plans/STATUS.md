...
- **Memory-driven default-sharing (ADR-0002 item 8 / ADR-0001 item 7) — IN FLIGHT.** Graduate the
  ...
  - **DS-4 tasks** (tasks-agent `OutboundHttpConfig` wraps `TasksSharingPolicy` in `LearnedSharingPolicy`
    + learning-enabled `SharingResolver` + `SharingLearningClient` bean; `TaskCapturer` routing; pure wiring):
    **this PR.**
  - **DS-4 remaining** — nutrition / docs, one per PR — each just wraps its static policy the same
    way. **Next.**
  - DS-N (deferred) confirm-on-ambiguity. See the ADR.
  - DS-0 (design) + DS-1 (store) + DS-2 (engine) + DS-3 (calendar) + DS-4 finance shipped → [HISTORY.md](HISTORY.md).
