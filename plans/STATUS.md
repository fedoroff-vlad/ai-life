...
## Now
- **Runtime consolidation ([#584](https://github.com/fedoroff-vlad/ai-life/issues/584), [ADR-0006](adr/ADR-0006-runtime-topology-footprint.md)) — Path B / B1 STARTED (before the Mac, owner direction 2026-09-08).** Goal: collapse ~30 JVMs (~10–15 GB) into a few co-hosting JVM hosts (~2–3 GB), zero contract change. A spike found the prerequisite: MCP modules ship an executable Boot **fat jar** as their main artifact, so a host can't depend on them → decouple packaging first (`exec` classifier → plain classes jar main + `-exec.jar` for Docker). **3a (packaging enabler, pilot `mcp-briefing`+`mcp-travel`) DONE (PR#656)** + **3b (`deploy/domain-mcp-host` boots two contexts in one JVM — `DomainMcpHostFootprintIntegrationTest`) DONE (PR#657)** + **3c (resident Domain-MCP-hot host) DONE:** 3a `exec`-classifier rolled out to the resident hot set (`mcp-caldav`·`mcp-finance`·`mcp-tasks`·`mcp-web`·`mcp-media-processing`) and `domain-mcp-host` repointed to `RESIDENT_HOT` (those five), footprint IT now boots all five side-by-side. Mechanism proven on the real hot set. **Next:** (i) the RAM-delta measurement (env-wired host `main` + `measure-footprint.sh` on the running stack — the "~2–3 GB vs ~10–15 GB" number, Mac-gated), (ii) the remaining hosts — Agent-hot, Platform-hot, cold host-units (same mechanism, per-tier lists; ADR-0006 item 4). Spec → [topology-map.md](topology-map.md) §Slice 3. Native/GraalVM (path C) stays the later, mac-gated lever.
