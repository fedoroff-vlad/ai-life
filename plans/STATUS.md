...
## Now
- **inventory domain ([inventory.md](inventory.md)) — spec accepted, IN-a IN FLIGHT (owner pick 2026-09-22).** New
  14th domain: "где что лежит" — photograph things into a container, the container carries a QR label, scanning
  (or photographing) the label returns a card of what is inside + where it stands. Driver is an imminent move;
  scope is permanent storage organisation (the move is two nullable columns, `status` + `destination`). Spec
  merged first per "flag before code" ([#671](https://github.com/fedoroff-vlad/ai-life/pull/671)); design reuses
  the route-lock (packing session), ADR-0004 (edits), ADR-0002 (sharing), memory-service (semantic recall) and
  `libs/doc-render` (card), adding exactly **one** new capability tool (`decode_qr` on `mcp-media-processing`,
  IN-b) and a `box_` prefix on the gateway's existing `/start` deep-link (IN-f). **IN-a** = `mcp-inventory`
  (port 8127) + the `inventory` schema (`120-inventory.yml`): zones → containers → items, `getContainerByToken`
  as the scan read, trigram `searchItems` returning each hit's container + zone. **Next after IN-a:** IN-b
  (`decode_qr`), then IN-c (agent + packing session). Label printing stays deferred until the printer exists —
  the owner is buying in the ≤4k class (direct thermal, TSPL), see [inventory.md](inventory.md) §Deferred.
