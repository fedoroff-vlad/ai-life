...
## Now
- **inventory domain ([inventory.md](inventory.md)) — IN FLIGHT; the scan path is closed and the whole loop is correctable in chat (IN-a…IN-g2 shipped + goldens); only appending to a closed box (IN-g3) and semantic recall (IN-e2) are left.**
  ...
  move; scope is permanent storage organisation. Spec merged first per "flag before code"
  ([#671](https://github.com/fedoroff-vlad/ai-life/pull/671)); shipped since then — **IN-a** the
  `mcp-inventory` store · **IN-b** `decode_qr` on `mcp-media-processing` ([media.md](media.md) §MP-f) ·
  **IN-c** the `box-packer` packing session on the shipped route-lock · **IN-d** the printed identity (QR
  label + doc-render card) · **IN-e** `item-finder` ("где лежит X", trigram half) · **IN-f1/f2** the scan
  path: `/start box_<token>` prefix dispatch + a front-door QR read of a captionless photo, both dispatched
  deterministically through the hub's C1 `invoke` (no classifier turn on a sticker), closed by
  `E2EInventoryScanFlowTest`. Detail → [inventory.md](inventory.md) + [HISTORY.md](HISTORY.md).
  **The goldens are written and green** (7 tests on `qwen3:8b`, run twice for stability): routing over the
  four trigger-less skills + the `box-packer` open/close extract + the `item-finder` query distil + the
  `box-label`/`box-card` container distil. Until then every inventory test fed the parser a JSON answer
  written by hand, so nothing could tell whether a real model routes or extracts correctly — the table in
  [inventory.md](inventory.md) §Golden tests records what each one catches.
  **IN-g1 (correcting a container) DONE** — the path a real move needs: "коробка B-07 теперь на даче"
  (zone move) · "распаковал B-07" (status) · "переименуй B-12 в «зимние вещи»", one `box-editor` skill on
  the shared ADR-0004 `PickConfirmActRunner`, confirm-gated (да/нет buttons) and patching **only** the
  named fields — `code`/`qr_token` never move, so a sticker already on the box keeps resolving. A store of
  boxes goes stale the moment the boxes do; without this the owner stops trusting the domain.
  **IN-g2 (taking a thing out of its box) DONE** — "убери из коробки старый чайник" on the same runner
  (a plain delete flow, so the wording is free). Two decisions: the candidate pool is the **search over
  the distilled phrase**, not the whole store (hundreds of things after a move fit neither the runner's
  cap nor a model's attention), and that distil lifted into `find/ItemQuery` on its second consumer.
  **Next:** **IN-g3** (appending to a closed box — "добавь в B-07 гирлянду" +photo, without reopening a
  session) · **IN-e2** (semantic recall — an item note seed on the write path, mirroring docs SB-5). Label printing stays deferred until the printer exists — the owner is buying in
  the ≤4k class (direct thermal, TSPL), see §Deferred.

- **Runtime consolidation ([#584](https://github.com/fedoroff-vlad/ai-life/issues/584), [ADR-0006](adr/ADR-0006-runtime-topology-footprint.md)) — Path B / B1 STARTED (before the Mac, owner direction 2026-09-08).** Goal: collapse ~30 JVMs (~10–15 GB) into a few co-hosting JVM hosts (~2–3 GB), zero contract change. A spike found the prerequisite: MCP modules ship an executable Boot **fat jar** as their main artifact, so a host can't depend on them → decouple packaging first (`exec` classifier → plain classes jar main + `-exec.jar` for Docker). **3a (packaging enabler, pilot `mcp-briefing`+`mcp-travel`) DONE (PR#656)** + **3b (`deploy/domain-mcp-host` boots two contexts in one JVM — `DomainMcpHostFootprintIntegrationTest`) DONE (PR#657)** + **3c (resident Domain-MCP-hot host) DONE:** 3a `exec`-classifier rolled out to the resident hot set (`mcp-caldav`·`mcp-finance`·`mcp-tasks`·`mcp-web`·`mcp-media-processing`) and `domain-mcp-host` repointed to `RESIDENT_HOT` (those five), footprint IT now boots all five side-by-side. Mechanism proven on the real hot set. **3d (resident Agent-hot host) DONE:** 3a `exec`-classifier rolled out to the six agents (`calendar`·`finance`·`tasks`·`notes`·`coordinator`·`researcher`) + new `deploy/agent-host`. The agent tier needed a *second* collision fixed beyond the `application.yml` skip — every agent's `AGENT.md` sat at `classpath:/AGENT.md`, so co-hosted contexts cross-loaded one persona; owner-chosen fix = each `AGENT.md` now lands at a per-agent `manifest/<name>/AGENT.md` via the module `pom.xml` `<targetPath>` (file stays at module root) + `manifest-classpath` repointed. `AgentHostFootprintIntegrationTest` boots all six, asserting each loaded its own persona. **3e (resident Platform-hot host) DONE:** 3a `exec`-classifier rolled out to the seven platform services (`gateway-telegram`·`orchestrator`·`profile-service`·`notifier-service`·`scheduler-service`·`conversation-service`·`media-service`) + new `deploy/platform-host`. The platform tier is DB-bound and mixes web stacks, so beyond the `application.yml` config-name skip the launcher **re-supplies `web-application-type` per module** (reactive: gateway/orchestrator/notifier; servlet: the rest) — load-bearing because `scheduler-service` carries both `spring-web` and test-only `webflux` (auto-detect would be ambiguous). `PlatformHostFootprintIntegrationTest` boots all seven side-by-side (Testcontainers PG wired to every context + a live SeaweedFS for media-service's boot-time `ensureBucket`; the two `@Scheduled` ticks kept quiet; gateway token-less). Isolated singletons `memory-service`+`llm-gateway` are not consolidated. **This completes the resident tier** (Domain-MCP-hot 3c + Agent-hot 3d + Platform-hot 3e). **3f (first cold host-unit — the Docs unit) DONE:** 3a `exec`-classifier on `docs-agent`+`mcp-docs` + new `deploy/docs-host` — **the first host that mixes the agent tier and its domain-MCP in one JVM** (cold grouping = co-usage affinity, unlike the resident agent/MCP split). Launcher re-supplies per-module `web-application-type` (docs-agent reactive, mcp-docs servlet) **and** the agent's `agent.skills-classpath` (config-name skip drops it, and the runtime fails startup if AGENT.md declares skills the empty registry never loaded); single agent → no manifest-path collision. `DocsHostFootprintIntegrationTest` boots both side-by-side (MCP client off = co-residency proof, not the agent→MCP SSE binding). **3g (second cold host-unit — the Content unit) DONE:** 3a `exec`-classifier rolled out to `creator-agent` + its domain-MCP `mcp-creator` + the three trend capability-MCPs it binds (`mcp-youtube`·`mcp-reddit`·`mcp-feeds`) + new `deploy/content-host` boots all five side-by-side — same single-agent+MCPs shape as Docs (3f), now a **5 JVMs → 1** cluster. Launcher re-supplies per-module `web-application-type` (agent + the three capability-MCPs webflux-only → reactive; `mcp-creator` carries both `spring-web`+`webflux` like `mcp-docs` → pinned servlet) + the agent's `agent.skills-classpath`; single agent → no manifest-path collision. `ContentHostFootprintIntegrationTest` boots all five (creator-agent MCP client off = co-residency proof). **3h (third cold host-unit — the Lifestyle unit; first cold unit with >1 agent) DONE:** 3a `exec`-classifier on three agents (`stylist`·`nutritionist`·`chef`) + their domain-MCPs (`mcp-wardrobe`·`mcp-nutrition`) + two capability-MCPs (`mcp-image-gen`·`mcp-food-data`) + new `deploy/lifestyle-host` boots all seven (**7 JVMs → 1**). Three agents share the classloader → carries the **per-agent manifest-path fix (3d)**: each `AGENT.md` → `manifest/<name>/AGENT.md` (pom `<targetPath>` + yml `manifest-classpath`), launcher points each context at its own persona (skills classpath shared — nutritionist+chef both scan `skills/nutrition/*`). Web-type re-supply: agents + capability-MCPs reactive; `mcp-wardrobe`+`mcp-nutrition` carry web+webflux → pinned servlet. `LifestyleHostFootprintIntegrationTest` asserts each agent loaded its own persona. **3i (fourth cold host-unit — the Brief+Travel unit; multi-agent) DONE:** two agents (`briefing`·`travel`) + their domain-MCPs (`mcp-briefing`·`mcp-travel`, already exec-classified in the 3a pilot) + two capability-MCPs (`mcp-weather`·`mcp-travel-search`) + new `deploy/brief-travel-host` boots all six (**6 JVMs → 1**). exec-classifier added to the four not-yet-done modules (both agents + the two capability-MCPs). Multi-agent → per-agent manifest-path fix (3d) as in 3h (each `AGENT.md` → `manifest/<name>/AGENT.md`, pom `<targetPath>` + main & test yml). Web-type: agents + capability-MCPs reactive; `mcp-briefing`+`mcp-travel` carry web+webflux → pinned servlet. `BriefTravelHostFootprintIntegrationTest` asserts each agent loaded its own persona. **3j (fifth cold host-unit — the Finance-aux unit; first MCP-only cold unit) DONE:** 3a `exec`-classifier rolled out to the four aux MCPs — two schema-less capability-MCPs (`mcp-market-data`·`mcp-chart-render`) + two import domain-MCPs (`mcp-money-pro-import`·`mcp-ics-import`) + new `deploy/finance-aux-host` boots all four side-by-side (**4 JVMs → 1**). First cold unit with **no agent** → MCP-only cluster grouped by co-usage affinity; like the resident Domain-MCP-hot host (3c) it needs neither the per-agent manifest-path fix (3d) nor the `agent.skills-classpath` re-supply, only the `application.yml` config-name skip + web-type pin (market-data+chart-render reactive; money-pro-import+ics-import carry web+webflux → pinned servlet). The two import MCPs are DB-bound → IT wires the shared Testcontainers PG at `ddl-auto=none` (no schema needed to boot). `FinanceAuxHostFootprintIntegrationTest` boots all four on distinct ports, each carrying only its own application bean. **Next:** (i) the RAM-delta measurement across the resident hosts (env-wired host `main` + `measure-footprint.sh` on the running stack — the "~2–3 GB vs ~10–15 GB" number, Mac-gated), (ii) the one remaining cold host-unit — Coach (parked #289; same mechanism when the epic thaws; ADR-0006 item 4). Spec → [topology-map.md](topology-map.md) §Slice 3. Native/GraalVM (path C) stays the later, mac-gated lever.
