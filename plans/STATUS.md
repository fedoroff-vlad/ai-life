...
## Now
- **inventory domain ([inventory.md](inventory.md)) — IN FLIGHT; the scan path is now closed (IN-a…IN-f2 shipped, incl. the domain's E2E closer), next slice is an owner pick.**
  ...
  **Next (owner pick):** **IN-g** (edit / append / delete / zone-move on the ADR-0004 runner — the
  correction path a real move needs: "убери оттуда X", "коробка B-07 теперь на даче", "распаковал B-07") ·
  **IN-e2** (semantic recall — an item note seed on the write path, mirroring docs SB-5). Label printing
  stays deferred until the printer exists — the owner is buying in the ≤4k class (direct thermal, TSPL),
  see §Deferred.
