...
- **inventory domain ([inventory.md](inventory.md)) — IN FLIGHT; store + packing + find + label/card landed, the scan path (IN-f) next (owner pick 2026-09-22).**
  ...
  container + its zone; trigram half only) and **IN-d** (the printed identity: a deterministic ZXing QR
  of the `t.me/<bot>?start=box_<token>` deep link stored in media-service + the `libs/doc-render`
  container card — code · label · zone · status · photo gallery · item list; both issued **on close**
