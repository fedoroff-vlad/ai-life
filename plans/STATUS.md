...
## Now
- **inventory domain ([inventory.md](inventory.md)) — IN FLIGHT; store landed, agent side next (owner pick 2026-09-22).**
  ...
  ([#671](https://github.com/fedoroff-vlad/ai-life/pull/671)), then **IN-a** (the `mcp-inventory` store) — both
  shipped, as are **IN-b** (`decode_qr` on `mcp-media-processing` — ZXing, no engine seam because it needs
  no native lib or model; [media.md](media.md) §MP-f) and **IN-c** (`inventory-agent` port 8128 + the
  `box-packer` packing session **on the shipped route-lock** — open a box, every photo becomes a
  vision-captioned item in it, close it) and **IN-e** (`item-finder` — "где лежит X" answers with the
  container + its zone; trigram half only). Detail → [inventory.md](inventory.md) + [HISTORY.md](HISTORY.md).
  **Next: IN-d** — the QR label image + the `libs/doc-render` container card (what the owner prints and
  what a scan will show), then IN-f (the scan path: `box_` prefix on the gateway's `/start` + a
  photographed label). Also owed: **IN-e2** (semantic recall — an item note seed, mirroring docs SB-5) and
  the domain's **goldens**, none of which exist yet (see [inventory.md](inventory.md) §Golden tests —
  they need a real model run, so they are their own slice). Label printing stays deferred until
  the printer exists — the owner is buying in the ≤4k class (direct thermal, TSPL), see §Deferred.
