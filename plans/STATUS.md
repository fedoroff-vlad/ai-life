...
- **inventory domain ([inventory.md](inventory.md)) — IN FLIGHT; store landed, agent side next (owner pick 2026-09-22).**
  ...
  shipped, detail → [inventory.md](inventory.md) + [HISTORY.md](HISTORY.md). **Next: IN-b** (`decode_qr` +
  `/internal/qr` on `mcp-media-processing` — the OCR twin, the domain's only new capability tool), then **IN-c**
  (`inventory-agent` + the `box-packer` packing session on the route-lock). Label printing stays deferred until
