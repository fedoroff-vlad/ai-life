...
- **inventory domain ([inventory.md](inventory.md)) — IN FLIGHT; store landed, agent side next (owner pick 2026-09-22).**
  ...
  shipped, as is **IN-b** (`decode_qr` + `/internal/qr` on `mcp-media-processing` — ZXing, no engine seam
  because it needs no native lib or model; see [media.md](media.md) §MP-f). Detail →
  [inventory.md](inventory.md) + [HISTORY.md](HISTORY.md). **Next: IN-c** — `inventory-agent` + the
  `box-packer` packing session on the route-lock (the first slice the owner can actually use in Telegram).
  Label printing stays deferred until
