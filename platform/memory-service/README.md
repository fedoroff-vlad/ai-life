...
## Roadmap (deferred)
- **Ambient / intuitive capture (in progress):** the memory-from-chat path (`CaptureService`) now has a **third output** — curated **ambient notes** — so the note tier fills itself without the "запомни" keyword. Phased plan (AC-1..5) in [plans/ambient-capture.md](../../plans/ambient-capture.md). **AC-1..AC-4 landed (classify → auto-save explicit / approve inferred / dedup), all flag-gated. AC-5 (merge/supersede) in progress: AC-5a `NoteReconciler` decision engine landed here (enrich/supersede/skip, fail-safe → skip); AC-5b wires it into the write path (next).**
