...
## Roadmap (deferred)
- **Ambient / intuitive capture (in progress):** the memory-from-chat path (`CaptureService`) now has a **third output** — curated **ambient notes** — so the note tier fills itself without the "запомни" keyword. Phased plan (AC-1..5) in [plans/ambient-capture.md](../../plans/ambient-capture.md). **AC-1..AC-3 landed; AC-4 (approval of inferred, proactive push + resume) — capture side AC-4b done here, resume side AC-4a in notes-agent, all flag-gated.** Next: AC-4c E2E loop closer, then AC-5 merge/supersede.
