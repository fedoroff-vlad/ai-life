...
wants to try it. This is a **light snapshot (last refreshed 2026-08-08)** — much has shipped since it was
...
- **Real LLM (Stage 5)** — **unblocked / closed** ([#199](https://github.com/fedoroff-vlad/ai-life/issues/199)) via local Ollama (`qwen3:8b`). Opt-in, CI-skipped **golden tests** (structure-not-text) now cover all domain agents + the orchestrator, sharing `libs/golden-test-support`. CI default stays `mock` (fast, no model); golden runs on demand against a running llm-gateway (`GOLDEN_LLM=1`).
