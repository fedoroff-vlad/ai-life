...
- `web/IntentController` — `POST /agents/finance/intent`. An `image` attachment → `ReceiptParser`; a `file` (document) attachment → `MoneyProCsvImporter`; otherwise delegates to `IntentRouter` (PR35). Returns `{agent, text, llmModel}`.
