...
- `receipt-parser` (intent-invoked, not a wake trigger) — extracts amount/currency/merchant/date from a receipt photo via the `vision` channel, shows the draft and **writes only after the user confirms** ("да"), via the conversation route-lock / resume mechanism (Stage 4 / A4). Lives at [skills/finance/receipt-parser/SKILL.md](../../skills/finance/receipt-parser/SKILL.md).
