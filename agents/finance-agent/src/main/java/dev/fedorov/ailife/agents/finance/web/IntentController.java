...
public class IntentController {
    ...
    public IntentController(IntentRouter router, ReceiptParser receiptParser, AgentManifest manifest) {
        this.router = router;
    ...
    public Mono<IntentResponse> intent(@RequestBody NormalizedMessage message) {
        // A photo attachment takes the receipt-parser path; otherwise the normal
        // text routing (LLM tool-call or chat) applies.
        return imageAttachment(message)
                .map(att -> receiptParser.parse(message, att.storageUri()))
                .orElseGet(() -> router.route(message.text())
