 ...
 * pattern (default: {@code classpath*:skills/finance/} *{@code /SKILL.md}) and
 * parses them into {@link Skill}s. The repo's {@code skills/finance/} dir is
 * copied onto the classpath at build time (see {@code pom.xml} {@code <resources>}).
 *
 * <p>An empty registry is valid — this PR ships no skills yet; the trigger
 * controller treats every kind as unknown until a SKILL.md ships for it.
 */
@Configuration
public class SkillLoader {
    ...
    public SkillRegistry skillRegistry(
            @Value("${finance-agent.skills-classpath:classpath*:skills/finance/*/SKILL.md}")
                    String pattern) {
