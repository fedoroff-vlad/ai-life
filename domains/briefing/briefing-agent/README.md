...
**briefing-preferences flow** (BR-c):
...
- **BR-d (next) — the `digest` flow.** Resolve the profile → gather the enabled sections on the
  `Coordinator` → one `briefing-composer` synthesis → the digest.

## Endpoints

- `POST /agents/briefing/intent` (body `NormalizedMessage`) → `IntentResponse` — the orchestrator's
  entry point. Preferences cue → briefing-profiler; else chat (the digest flow arrives in BR-d).
...
| `MCP_WEB_URL` | `http://mcp-web:8098` | shared web/news capability (BR-d) |
...
- `config/OutboundHttpConfig` — one `clone()`d `WebClient` per dependency (`mcpBriefing`, `mcpWeather`,
  `mcpWeb`); the shared `profile/notifier/memory` clients come from `agent-runtime`.
- `web/IntentController` — `POST /agents/briefing/intent`; preferences cue → profiler, else chat.
