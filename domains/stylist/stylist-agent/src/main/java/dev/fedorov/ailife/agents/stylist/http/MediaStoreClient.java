...
/**
 * Uploads a rendered deliverable (the analysis / capsule HTML) to media-service and returns its
 * catalogue entry, so the flow can hand the user a stable link to open on any device. Mirrors
 * gateway-telegram's {@code MediaServiceClient} upload (multipart {@code POST /v1/media}). The
 * stored MIME type is the renderer's ({@code text/html}), so opening the link renders the page.
 */
@Component
