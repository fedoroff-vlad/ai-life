...
/**
 * Uploads a rendered deliverable (the nutrition-analysis HTML, later the basket/ration reports) to
 * media-service and returns its catalogue entry, so the flow can hand the user a stable link to open
 * on any device. Mirrors stylist-agent's {@code MediaStoreClient} (multipart {@code POST /v1/media}).
 * The stored MIME type is the renderer's ({@code text/html}), so opening the link renders the page.
 */
@Component
