...
the **nutrition-analysis board** (NU-e). Remaining flows replace the fallback branch-by-branch:
...
- `POST /agents/nutritionist/intent` (body `NormalizedMessage`) → `IntentResponse` — the
  orchestrator's entry point. A photo → food-log; a profile cue → diet-profiler; an analysis cue →
  nutrition-analysis; a food-log cue → food-log; else chat.
...
- `web/IntentController` — `POST /intent` (photo → food-log; profile cue → diet-profiler; analysis
  cue → nutrition-analysis; food-log cue → food-log; else chat).
...
meal-logger, diet-profiler, nutrition-analyst`. ⚠️ Carries the infant/medical-safety rule (general
guidance only + pediatrician caveat).
