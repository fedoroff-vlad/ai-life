...
/**
 * Outbound HTTP destinations the chef talks to. {@code mcpNutritionUrl} reads the ration / diet
 * context (the nutrition domain-MCP); {@code mcpWebUrl} is the shared web capability the recipe
 * flow searches over its {@code /internal/*} passthrough. {@code mediaServiceUrl} stores the
 * rendered recipe-card HTML. The profile / notifier / memory URLs back the shared
