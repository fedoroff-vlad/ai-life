package dev.fedorov.ailife.agents.chef.http;
...
/**
 * Uploads a rendered recipe card (the CH-b HTML deliverable) to media-service and returns its
 * catalogue entry, so the flow can hand the user a stable link to open on any device. Mirrors
 * nutritionist-agent's {@code MediaStoreClient} (multipart {@code POST /v1/media}); the stored MIME
 * type is the renderer's ({@code text/html}), so opening the link renders the page.
 */
@Component
