...
## Status (SB-4 + resurfacing R-b + ambient-approve AC-4 + lists LI-a + undo H + note-delete/note-edit H.2 + memory-review MQ-1 + fact-forget MQ-2)
...
- **`memory-review`** (`domains/knowledge/skills/memory-review/SKILL.md`) — routing-only (no structured
  output): selects the memory-review digest flow when the user asks what the assistant remembers about them
  or the household ("что ты про меня запомнил", MQ-1 / road-test #488). Distinct from `note-finder` (which
  recalls one specific topic).
- **`fact-forget`** (`domains/knowledge/skills/fact-forget/SKILL.md`) — strict-JSON pick of which remembered
  **fact** (the `memory.memories` tier, not a note) to forget or correct, optionally with the corrected fact
  (`{"pick":n}` / `{"pick":n,"correction":"…"}` / `{"ambiguous":[…]}` / `{}`), for the confirm-gated
  forget/correct-a-fact flow (MQ-2, road-test #488). "забудь, что …" forgets; "это неверно, на самом деле …"
  corrects. Distinct from `note-delete`/`note-edit` (which target saved notes) and `memory-review` (read-only).
...
| `MEMORY_SERVICE_URL` | `http://memory-service:8087` | the knowledge base — `/v1/notes` (store, get, backlinks, resurface) + `/v1/memories` (recall, list, get, delete, write — the fact tier for the MQ-1 digest + MQ-2 forget/correct). |
...
- `review/MemoryReviewer` — the **memory-review digest** flow (MQ-1, road-test #488): "что ты про меня / про нас запомнил" → gather the household's notes (`NoteClient.list`, `type=list` excluded) + facts (`MemoryClient.listMemories`, scoped to owner + household-shared, `source=note` seeds excluded) and fold them into one readable digest ending with the drop/correct verbs ("удали заметку про …", "забудь, что …"). Read-only + deterministic — no LLM turn (routing already picked it); every stage soft-fails (the clients downgrade to empty, so a memory-service blip just thins the digest).
...
- `intent/FactForgetter` — the user-facing **forget/correct-a-fact** flow (MQ-2, road-test #488 — the fact-tier analog of `NoteDeleter`/`NoteEditor`), behind a **confirm-before-forget/correct** gate. `forget(msg)`: read the household's remembered facts (`MemoryClient.listMemories`, scoped to owner + household-shared, enumerated by recency; `source=note` seeds excluded — those are note-flow's job) → LLM pick (`fact-forget` SKILL, temperature 0, numbered candidates) returning `{"pick":n}` / `{"pick":n,"correction":"…"}` / `{"ambiguous":[…]}` / `{}` → a single match replies with a `pendingAction` asking to confirm (forgets nothing); ambiguous lists, none/miss asks. `resume(req)`: an affirmative acts — **forget** (no correction) → `MemoryClient.deleteMemory` (`DELETE /v1/memories/{id}`); **correct** → re-read the row for its household/user (`MemoryClient.getMemory` — mirrors how `TransactionEditor`/`NoteEditor` re-read their target), forget it, then write the corrected fact under the same scope (`MemoryClient.remember`, `source=correction`) — anything else leaves it; either reply clears the lock. The pick→confirm→act loop is the shared `agent-runtime` `PickConfirmActRunner` (ADR-0004); this class is the fact adapter (`candidates`/`view`/`act`/`Phrasing`, `source=note` exclusion in the read). Every stage soft-fails.
- `intent/NotesIntentRouter` — the in-agent router (#475): one llm-gateway turn via the shared `agent-runtime` `SkillClassifier` offers the seven intent skills as one `skill` choice (their SKILL.md descriptions are the routing SSOT) → dispatch to `NoteFinder`/`ListManager`/`NoteWriter`/`NoteDeleter`/`NoteEditor`/`MemoryReviewer`/`FactForgetter`, or the `NotesChat` fallback (blank / unknown-skill / non-JSON / LLM error all soft-fail to chat). Replaced the old keyword-cue heuristic.
- `web/IntentController` — thin: delegates `/intent` to `NotesIntentRouter`; `web/ManifestController`.
- `web/ResumeController` — `POST /agents/notes/resume`; dispatches on `pendingAction.flow` (`ambient-approve` → `AmbientApprover`, `note-delete-confirm` → `NoteDeleter.resume`, `note-edit-confirm` → `NoteEditor.resume`, `fact-forget-confirm` → `FactForgetter.resume`).
