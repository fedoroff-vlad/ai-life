...
- `intent/GoldenCalendarRoutingTest` — **opt-in** routing golden (#544, `@GoldenLlmTest` / `GOLDEN_LLM`, excluded from CI): replays the exact `CalendarIntentRouter` prompt against a **real model**, asserting *structure not text* — the classifier returns well-formed routing JSON (valid `action`, non-hallucinated skill), and crisp phrasings route to the right flow (`запиши встречу…` → `event-capture`, `отмени…` → `event-cancel`, `перенеси…` → `event-move`; a non-event message → chat). Run: `scripts/golden.sh -pl domains/calendar/calendar-agent -Dtest=GoldenCalendarRoutingTest` (~2–3 min, CPU-only Ollama).
