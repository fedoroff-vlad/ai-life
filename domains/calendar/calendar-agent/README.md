...
- `http/CaldavEventClient` — `createEvent(CreateEventInput)` POSTs mcp-caldav `/internal/event`; `eventsInWindow(household, from, to)` GETs `/internal/events` (backs the `create_event` double-booking sanity check, #485). Used by the `create_event` action.
