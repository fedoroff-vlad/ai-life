 ...
 * event-cancel} delete, {@code event-move} reschedule) + the chat fallback + the intro/decide framing, and
 * the shared router owns the LLM round-trip, the {@link SkillClassifier} parse, and the soft-fail-to-chat
 * dispatch. Each intent skill's SKILL.md {@code description} is the routing SSOT (a paraphrase outside any
 * keyword list routes correctly). The birthday/gift skills stay off this map — they are wake-driven, not
 * user-routable, so the router never advertises them.
 */
@Component
public class CalendarIntentRouter {
    ...
    private static final String EVENT_MOVE = "event-move";
    ...
    public CalendarIntentRouter(LlmClient llm, SkillRegistry skills, SkillClassifier classifier,
                                ...
                                EventMover mover, CalendarChat chat) {
        ...
        flows.put(EVENT_MOVE, mover::move);
        this.router = new SkillRouter(llm, skills, classifier, manifest,
                ...
                "Decide: does the user want to run a skill (put an event on the calendar, cancel one, or move one) or just talk?",
                flows, chat::reply);
    }
