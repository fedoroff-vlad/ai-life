 ...
 * notes-agent: it supplies the calendar-specific dispatch map ({@code event-capture}) + the chat fallback +
 * the intro/decide framing, and the shared router owns the LLM round-trip, the {@link SkillClassifier}
 * parse, and the soft-fail-to-chat dispatch. Each intent skill's SKILL.md {@code description} is the routing
 * SSOT (a paraphrase outside any keyword list routes correctly). The birthday/gift skills stay off this map
 * — they are wake-driven, not user-routable, so the router never advertises them. HC-3/HC-4 add
 * {@code event-cancel} / {@code event-move} to the map.
 */
@Component
public class CalendarIntentRouter {
    ...
    public CalendarIntentRouter(LlmClient llm, SkillRegistry skills, SkillClassifier classifier,
                                AgentManifest manifest, EventCapturer capturer, CalendarChat chat) {
        ...
        this.router = new SkillRouter(llm, skills, classifier, manifest,
                ...
                "Decide: does the user want to run a skill (put an event on the calendar) or just talk?",
                flows, chat::reply);
    }
