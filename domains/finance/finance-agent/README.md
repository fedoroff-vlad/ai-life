...
| POST   | `/agents/finance/actions/{action}`         | inter-agent action (Stage 4 / Track D); `get_gift_budget` → mcp-finance `/internal/gift-budget`, returns `{hasGiftBudget, amount?, currency?, remaining?}` |
...
| `MCP_FINANCE_URL` | `http://mcp-finance:8092` | `GET /internal/budget-status` for `budget.alert` enrichment; `GET /internal/accounts` + `POST /internal/transaction` for the receipt flow; `GET /internal/gift-budget` for the `get_gift_budget` action. |
...
- `web/ActionController` (Stage 4 / Track D, D2b) — `POST /agents/finance/actions/{action}`; inter-agent action endpoint (consumer side of the orchestrator invoke primitive). `get_gift_budget` forces `householdId` from the envelope, reads `GiftBudgetClient`, and returns `{hasGiftBudget, amount?, currency?, remaining?}`. Always replies an `AgentActionResult` (structured `ok=false` on missing household / mcp-finance down, never an HTTP error). Mirrors calendar-agent's `create_event` action (C1c).
- `http/GiftBudgetClient` — `GET /internal/gift-budget?householdId=` against mcp-finance. 200 → `Optional.of(result)`; 404 → `Optional.empty()` (no Gifts budget — action returns `hasGiftBudget=false`); 5xx / timeout → propagated, action returns `ok=false`. Same per-status shape as `BudgetStatusClient`.
...
- `receipt-parser` (intent-invoked, not a wake trigger) — extracts amount/currency/merchant/date from a receipt photo via the `vision` channel, shows the draft and **writes only after the user confirms** ("да"), via the conversation route-lock / resume mechanism (Stage 4 / A4). Lives at [skills/finance/receipt-parser/SKILL.md](../skills/receipt-parser/SKILL.md).
