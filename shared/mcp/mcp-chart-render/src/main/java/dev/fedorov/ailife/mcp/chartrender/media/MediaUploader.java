package dev.fedorov.ailife.mcp.chartrender.media;
...
/**
 * Stores a rendered chart in media-service ({@code POST /v1/media} multipart) and returns its
 * catalogue entry, so the capability hands callers a stable media id (like any other media object —
 * a finance report embeds it inline). Mirrors {@code mcp-image-gen}'s {@code MediaUploader}.
 */
@Component
